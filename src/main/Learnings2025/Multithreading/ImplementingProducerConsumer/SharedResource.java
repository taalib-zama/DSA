...
public class SharedResource {
    ...
    public synchronized void produce(int item) throws Exception {
        //If buffer is full, wait for the consumer to consume an item
        while(sharedqueue.size() >= bufferSize){
            System.out.println("Buffer is full, producer is waiting for consumer to consume an item");
            wait();
        }
        //Add item to the queue
        sharedqueue.add(item);
        System.out.println("Produced item: " + item + ", Current buffer size: " + sharedqueue.size());
        //Notify the consumer that an item has been produced and space is available in the buffer
        notifyAll();
