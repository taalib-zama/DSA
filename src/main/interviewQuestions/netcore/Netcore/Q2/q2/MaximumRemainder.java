...
public class MaximumRemainder {
    ...
    public static int find_maximum_remainderUsingStream(int N,long [] A,long k) {
        // find max odd.
        OptionalLong maxOdd =  Arrays.stream(A)
                .filter(num -> num % 2 != 0)
                .max();
        //max even
        OptionalLong maxEven =  Arrays.stream(A)
                .filter(num -> num % 2 == 0)
                .max();

        // Calculate the maximum remainder
        if (maxOdd.isPresent() && maxEven.isPresent()) {
            return Math.toIntExact((maxOdd.getAsLong() + maxEven.getAsLong()) % k);
        }
